 ...
 */
import { MUSICAL_CONDUCTOR_EVENT_TYPES, MUSICAL_DYNAMICS, MUSICAL_TIMING, } from "../SequenceTypes.js";
...
export class BeatExecutor {
    ...
    beatExecutionQueue = [];
    constructor(eventBus, spaValidator, statisticsManager) {
    ...
    validateBeat(beat) {
        try {
            ...
            if (!beat.dynamics ||
                !Object.values(MUSICAL_DYNAMICS).includes(beat.dynamics)) {
                throw new Error("Beat must have valid dynamics");
            }
            if (!beat.timing ||
                !Object.values(MUSICAL_TIMING).includes(beat.timing)) {
                throw new Error("Beat must have valid timing");
            }
            if (!beat.errorHandling ||
                !["continue", "abort-sequence", "retry", "abort"].includes(beat.errorHandling)) {
...
//# sourceMappingURL=BeatExecutor.js.map