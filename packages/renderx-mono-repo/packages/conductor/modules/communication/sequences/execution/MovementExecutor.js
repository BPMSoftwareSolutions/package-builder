 ...
 */
import { MUSICAL_CONDUCTOR_EVENT_TYPES, MUSICAL_TIMING, } from "../SequenceTypes.js";
...
export class MovementExecutor {
    ...
    constructor(eventBus, spaValidator, statisticsManager) {
        ...
        this.beatExecutor = new BeatExecutor(eventBus, spaValidator, statisticsManager);
    }
    /**
    ...
    async handleBeatTiming(beat, sequence) {
        ...
        const effectiveTempo = typeof beat?.tempo === "number" && beat.tempo > 0
            ? beat.tempo
            : (typeof sequence.movements?.[sequence.movements.indexOf]
                ?.tempo === "number"
                ? sequence.movements?.[0]?.tempo
                : sequence.tempo) || 120;
