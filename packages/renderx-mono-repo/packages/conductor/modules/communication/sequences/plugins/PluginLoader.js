    ...
    async loadPluginModule(pluginPath) {
                ...
                if (!module?.sequence && !module?.handlers) {
                    try {
                        ...
                        const transformed = src
                            ...
                            .replace(/export\s+async\s+function\s+(\w+)\s*\(/g, "moduleExports.$1 = async function $1(")
                            .replace(/export\s+function\s+(\w+)\s*\(/g, "moduleExports.$1 = function $1(")
        ...
        if (isDev) {
            ...
            try {
                ...
                console.warn(`⚠️ Failed to load plugin from original path: ${pluginPath}. Error: ${originalError instanceof Error
        ...
        try {
            ...
            for (const strategy of resolutionStrategies) {
                try {
                    ...
                    console.log(`⚠️ Strategy failed: ${strategy} - ${strategyError instanceof Error
