...
import { PluginInterfaceFacade, } from "./plugins/PluginInterfaceFacade.js";
...
export class MusicalConductor {
    ...
    requestCorrelationMap = new Map();
    correlationActiveCounts = new Map();
    eventUnsubscribes = [];
    ...
    constructor(eventBus) {
        ...
        this.eventSubscriptionManager = new EventSubscriptionManager(eventBus, this.conductorCore.getSPAValidator());
        ...
        this.sequenceExecutor = new SequenceExecutor(eventBus, this.conductorCore.getSPAValidator(), this.executionQueue, this.statisticsManager);
        this.pluginManager = new PluginManager(eventBus, this.conductorCore.getSPAValidator(), this.sequenceRegistry);
        ...
        this.sequenceOrchestrator = new SequenceOrchestrator(eventBus, this.sequenceRegistry, this.executionQueue, this.sequenceExecutor, this.statisticsManager, this.sequenceValidator, this.sequenceUtilities, this.resourceDelegator);
        this.eventOrchestrator = new EventOrchestrator(eventBus);
        // Initialize API components
        this.conductorAPI = new ConductorAPI(this.sequenceOrchestrator, this.sequenceExecutor, this.executionQueue, this.statisticsManager, this.pluginInterface, this.sequenceRegistry, eventBus);
        ...
        try {
            ...
            console.log(`🎼 MusicalConductor v${v}: Initialized with core components`);
        }
        catch {
    ...
    play(pluginId, sequenceId, context = {}, priority = SEQUENCE_PRIORITIES.NORMAL) {
        try {
            ...
            if (count > 0) {
                console.log(`🎼 MusicalConductor.play: preserved ${count} callback(s) for correlationId=${correlationId}`);
            }
...
//# sourceMappingURL=MusicalConductor.js.map