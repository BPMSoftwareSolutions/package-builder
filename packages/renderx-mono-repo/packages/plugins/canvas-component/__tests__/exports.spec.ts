...
describe('@renderx/canvas-component package exports', () => {
  it('exposes a callable register() and deep symphony subpaths', async () => {
    let pkg: any;
    try {
      pkg = await import('@renderx/canvas-component');
    } catch (e) {
      const msg = String(e || '');
    ...
    const resizeStart = await import('@renderx/canvas-component/symphonies/resize/resize.start.symphony.ts');
    expect(typeof resizeStart.handlers).toBe('object');

    const create = await import('@renderx/canvas-component/symphonies/create/create.symphony.ts');
