let routes = {};
let loaded = false;
let interactionProvider = null;
export function setInteractionManifestProvider(p) {
...
export async function initInteractionManifest() {
    if (interactionProvider?.init) {
        try {
            await interactionProvider.init();
        }
        catch { }
    }
...
export function resolveInteraction(key) {
    if (interactionProvider) {
        try {
            return interactionProvider.resolveInteraction(key);
        }
        catch { }
    }
    if (!loaded && !interactionProvider) {
...
export function getInteractionManifestStats() {
    if (interactionProvider?.getStats) {
        try {
            return interactionProvider.getStats();
        }
        catch { }
    }
