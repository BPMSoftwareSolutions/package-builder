...
let startupProvider = null;
export function setStartupStatsProvider(p) { startupProvider = p; }
// Lightweight manifest presence & counts for host startup logging
export async function getPluginManifestStats() {
    if (startupProvider) {
        try {
            return await startupProvider.getPluginManifestStats();
        }
        catch { }
    }
    try {
...
export async function verifyArtifactsIntegrity(devOnly = true) {
    if (startupProvider?.verifyArtifactsIntegrity) {
        try {
            return await startupProvider.verifyArtifactsIntegrity(devOnly);
        }
        catch { }
    }
    try {
        ...
        if (devOnly && typeof process !== 'undefined' && process.env?.NODE_ENV === 'production')
            return null;
        // @ts-ignore
        if (typeof process !== 'undefined' && process.env?.RENDERX_DISABLE_INTEGRITY === '1')
            return null;
        let json = null;
        let mode = 'none';
        try {
        ...
        if (!json) {
            try {
                ...
                const rawTxt = await fs.readFile(integrityFile, 'utf-8').catch(() => null);
                if (rawTxt) {
        ...
        if (failed.length)
            console.warn('⚠️ Artifact integrity mismatch:', failed, '(mode:', mode, ')');
        else
            console.log('✅ Artifact integrity verified:', entries.length, 'files', '(mode:', mode, ')');
    }
    catch { }
    return null;
}
