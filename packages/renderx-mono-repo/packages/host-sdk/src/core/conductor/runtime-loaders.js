...
export const runtimePackageLoaders = {
...
export async function loadJsonSequenceCatalogs(conductor, pluginIds) {
    ...
    if (!plugins.length) {
        ...
        if (manifest && Array.isArray(manifest.plugins)) {
            ...
            try {
                ...
                if (catalogDirs.size) {
                    conductor._sequenceCatalogDirsFromManifest =
    ...
    try {
        console.log('🧭 Catalog discovery plugins:', plugins);
    }
    catch { }
    const isTestEnv = typeof import.meta !== "undefined" && !!import.meta.vitest;
    const forcedBrowser = typeof globalThis !== "undefined" &&
        globalThis.__RENDERX_FORCE_BROWSER === true;
    const isBrowser = (forcedBrowser || !isTestEnv) &&
        ...
        typeof globalThis.fetch === "function";
    const seen = new Set();
    const runtimeMounted = conductor._runtimeMountedSeqIds instanceof Set
        ? conductor._runtimeMountedSeqIds
        : new Set(Array.isArray(conductor._runtimeMountedSeqIds)
            ? conductor._runtimeMountedSeqIds
            : []);
    const mountFrom = async (seq, handlersPath) => {
        try {
            ...
            if (seen.has(seq.id)) {
                conductor.logger?.warn?.(`Sequence ${seq.id} already processed in loader; skipping`);
                try {
                    console.log('\u23ed\ufe0f Sequence already processed in loader; skipping:', seq?.id);
                }
                catch { }
                return;
            }
            ...
            try {
                if (isBrowser && isBareSpecifier(handlersPath) && runtimePackageLoaders[handlersPath]) {
                    try {
                        console.log('\uD83D\uDD0D Using runtimePackageLoader for', handlersPath);
                    }
                    catch { }
            ...
            if (!mod) {
                try {
                    if (isBrowser && isBareSpecifier(handlersPath) && /@renderx-plugins\/[^/]+\/symphonies\//.test(handlersPath)) {
                        ...
                        const loader = vendor?.getVendorSymphonyLoader?.(handlersPath);
                        if (typeof loader === 'function') {
                            try {
                                console.log('\uD83D\uDD0D Using vendor symphony loader for', handlersPath);
                            }
                            catch { }
                            mod = await loader();
                        }
            ...
            if (!mod) {
                try {
                    console.log('\u21AA\uFE0F Dynamic importing handlers module via spec:', spec);
                }
                catch { }
            ...
            if (!handlers) {
                ...
                try {
                    console.log('\u26a0\ufe0f No handlers export found at', handlersPath, 'for', seq?.id);
                }
                catch { }
            }
            if (alreadyMountedByRuntime && seq.pluginId !== 'LibraryComponentPlugin') {
                try {
                    console.log('\u23ed\ufe0f Sequence already mounted by runtime; skipping:', seq?.id);
                }
                catch { }
            }
            else {
                if (alreadyMountedByRuntime && seq.pluginId === 'LibraryComponentPlugin') {
                    try {
                        console.log('\ud83d\udd01 Forcing mount to ensure facade for LibraryComponentPlugin:', seq?.id);
                    }
                    catch { }
                }
                await conductor?.mount?.(seq, handlers, seq.pluginId);
                try {
                    console.log('\u2705 Mounted sequence from catalog:', seq?.id, 'plugin:', seq?.pluginId);
                }
                catch { }
                try {
                    runtimeMounted.add(seq.id);
                    conductor._runtimeMountedSeqIds = runtimeMounted;
                }
                catch { }
            }
            seen.add(seq.id);
            ...
            try {
                console.log('\u26a0\ufe0f Failed to mount sequence from catalog:', seq?.id, 'handlersPath:', handlersPath, 'error:', String(e));
            }
            catch { }
        }
    };
    for (const plugin of plugins) {
        const dir = plugin === "CanvasComponentPlugin"
            ? "canvas-component"
            : plugin === "CanvasPlugin"
                ? "canvas-component"
        ...
        try {
            try {
                console.log('📚 Loading catalog directory', dir, 'for plugin', plugin);
            }
            catch { }
            let entries = [];
            if (isBrowser) {
            ...
            if (!entries.length) {
                if (!isBrowser && artifactsDir) {
                    try {
                        ...
                        const cwd = procAny2 && typeof procAny2.cwd === "function"
                            ? procAny2.cwd()
                            : "";
            ...
            try {
                console.log('📖 Catalog entries for', dir, ':', Array.isArray(entries) ? entries.length : 0);
            }
            catch { }
            const tasks = entries.map(async (ent) => {
                ...
                if (isBrowser) {
                    try {
                        ...
                        try {
                            console.log('[90m[1m[36mFetching sequence[0m', { dir, file: ent.file, url: filePath });
                        }
                        catch { }
                        const seqRes = await fetch(filePath);
                        try {
                            console.log('[90m[1m[36mFetch status[0m', { url: filePath, ok: seqRes.ok, status: seqRes.status });
                        }
                        catch { }
                        if (seqRes.ok) {
                            seqJson = await seqRes.json();
                            try {
                                console.log('[90m[1m[36mLoaded JSON for[0m', { id: seqJson?.id, pluginId: seqJson?.pluginId });
                            }
                            catch { }
                        }
                    }
                        ...
                        try {
                            console.log('[90m[1m[36mFetch error[0m', String(e));
                        }
                        catch { }
                    }
                }
                if (!seqJson && !isBrowser && artifactsDir) {
                    try {
                        ...
                        const cwd = procAny3 && typeof procAny3.cwd === "function"
                            ? procAny3.cwd()
                            : "";
                ...
                if (!seqJson && !artifactsDir) {
                    ...
                    try {
                        console.log('\u2728 Fallback raw import used for', { dir, file: ent.file, id: seqJson?.id });
                    }
                    catch { }
                }
...
//# sourceMappingURL=runtime-loaders.js.map